...
0. **Личные кабинеты и задачи:**
   ...
   - `manager` («Менеджер») — задачи по всей сети и создание/изменение созданных им аккаунтов.
   - `employee` — внутренняя роль личного кабинета, не показываемая как должность; пользователь видит только свои задачи и персональные показатели iiko.
   - Аккаунт создаётся без привязки к торговой точке: точка выбирается при постановке каждой задачи.
